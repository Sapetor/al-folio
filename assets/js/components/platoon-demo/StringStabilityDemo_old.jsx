    ...
    case 'lamp':
      return (
        <g transform={`translate(${x}, 0)`}>
          <rect x="-2" y="50" width="4" height="60" fill={color} />
          <path
            d="M -2 50 L 2 50 L 5 45 L -5 45 Z"
            fill={color}
          />
          <circle cx="0" cy="43" r="5" fill={isDarkMode ? "#ffd" : "#ff8"} />
        </g>
      );
    case 'building':
      const height = 40 + (x % 20); // Deterministic height based on position
      return (
        <g transform={`translate(${x}, 0)`}>
          <rect
            x="-12"
...
const StringStabilityDemo = () => {
  ...
  // Generate background elements with different patterns
  const generateElements = () => {
    const elements = [];
    let id = 0;
    const totalWidth = 2400; // 3x view width

    // Add evenly spaced lights
    const lampSpacing = 300;
    for (let x = 0; x < totalWidth; x += lampSpacing) {
      elements.push({ id: id++, x: x, type: 'lamp' });
    }

    // Add randomly placed buildings
    for (let x = 0; x < totalWidth; x += 400) { // Sparser building placement
      if (Math.random() < 0.7) { // 70% chance of building
        const offset = Math.random() * 200 - 100;
        elements.push({ id: id++, x: x + offset, type: 'building' });
      }
    }

    // Add tree clusters
    for (let x = 0; x < totalWidth; x += 500) { // Space between potential clusters
      if (Math.random() < 0.6) { // 60% chance of cluster
        const baseX = x + Math.random() * 100;
        const clusterSize = Math.floor(Math.random() * 3) + 2; // 2-4 trees per cluster

        for (let i = 0; i < clusterSize; i++) {
          const treeOffset = Math.random() * 60 - 30; // Trees within ±30px of cluster center
          elements.push({
            id: id++,
            x: baseX + treeOffset,
            type: 'tree',
  ...
  const updateVehicles = (timestamp) => {
    ...
    // Move background based on current speed (independent of slider)
      ...
      <div className={`border rounded-lg overflow-hidden ${isDarkMode ? 'bg-gray-800' : 'bg-gray-100'}`}>
        ...
        >
          ...
          {bgElements.map((elem) => (
            <BackgroundElement
              key={elem.id}
              x={(elem.x + sceneryOffset) % 2400 - 800}
              type={elem.type}
