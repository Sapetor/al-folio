...
const BackgroundElement = ({ x, isDarkMode, scale = 1, yOffset = 0, type }) => {
  ...
  if (type === 'smallTree') return (
    <g transform={`translate(${x}, ${yOffset}) scale(${scale})`}>
      {/* Grass patch */}
      <rect x="-20" y="85" width="40" height="5" fill={isDarkMode ? "#2d4" : "#3a5"} rx="2" />
      <rect x="-4" y="90" width="8" height="30" fill={"#123"} />
      <path d="M -15 90 L 0 50 L 15 90 Z" fill={isDarkMode ? "#2d4" : "#3a5"} />
      <path d="M -12 70 L 0 35 L 12 70 Z" fill={isDarkMode ? "#2d4" : "#3a5"} />
    </g>
  );
...
const StringStabilityDemo = () => {
  ...
  const elementTypes = [
    { type: 'tree', scale: 1.0, yOffset: 0, speed: 'foreground', layer: 2 },    // Front trees
    ...
    { type: 'building', scale: 1.2, yOffset: 0, speed: 'background', layer: 1 }  // Buildings
  ];
